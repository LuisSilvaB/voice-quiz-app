...
const QuestionsPresentationModal:React.FC<Props> = ({questions, questionClear, ...toggle}) => {
  ...
  const revealQuestion = (questionId:string) => {
    setQuestionTargetId((state) => 
      questionId === state? null : questionId
    )
  }
  return createPortal(
    <motion.div
    ...
    >
      ...
      <div className="fixed bottom-0 left-0 right-0 top-0 z-30 m-auto box-border flex max-h-[600px] min-h-[300px] w-[1000px] flex-col gap-2 rounded-lg bg-white p-4 text-black">
        ...
        >
          {questions?.map((questionSelected: Question, index: number) => (
            <SwiperSlide key={index} className='flex flex-col justify-between'>
              <div className='w-full h-[510px] flex flex-col justify-between'>
                <div className="flex w-full flex-col justify-between">
                  ...
                  <div className="mt-3">
                    {questionSelected?.type === "true_or_false" ? (
                      <Chip
                        value={
                          questionSelected?.ID ? "Verdadero o Falso" : "Pregunta"
                        }
                        variant="ghost"
                    ...
                    {questionSelected?.type === "open_answer" ? (
                      <Chip
                        value={
                          questionSelected?.ID ? "Pregunta abierta" : "Pregunta"
                        }
                        variant="ghost"
                        color="green"
                        className="w-fit truncate text-ellipsis rounded-md border px-2 text-xs"
                ...
                <div className="flex items-end justify-between">
                  <div className="flex flex-col items-start gap-2">
                    ...
                    {questionSelected.ID === questionTargetId ? (
                      <p className="max-w-[700px] text-xl font-semibold text-gray-800">
                  ...
                  <Button
                    placeholder={""}
                    color="amber"
                    onClick={() => revealQuestion(questionSelected.ID)}
                  >
                    {questionSelected.ID === questionTargetId
                      ? "Ocultar Respuesta"
                      : "Revelar Respuesta"}
...
export default QuestionsPresentationModal