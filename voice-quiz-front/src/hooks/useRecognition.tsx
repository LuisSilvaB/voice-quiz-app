...
const useRecognition = () => {
  ...
  const fragments = useSelector(
    (state: RootState) => state.fragments.fragments,
  );
  const fragmentLoading = useSelector(
    (state: RootState) => state.fragments.fragmentLoading,
  );
  ...
  const onGenerateFragments = useCallback(async () => {
    const fragmentSize: number = currenSession.context ?? 300; // Reducido de 1000 a 300 para generar fragmentos más frecuentemente
    ...
    if (transcript && audioStream && !requestSent) {
      try {
        ...
        if (transcript.length - newProcessedCharacters >= fragmentSize) {
          const fragmentContent = transcript.slice(
            newProcessedCharacters,
            newProcessedCharacters + fragmentSize,
          );
          const formData = new FormData();
          formData.append(
            "documents",
            new Blob([fragmentContent], { type: "text/plain" }),
            "transcript.txt",
          );
